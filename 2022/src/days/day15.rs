...
fn p1(input: &str) -> eyre::Result<usize> {
    ...
    dbg!(&sensors);
    sensors.get_mut(0).unwrap().pos.0 += 1;
    sensors.get_mut(0).unwrap().beacon.0 += 1;
    todo!()
}
...
const __SENSOR_REGEX_INNER: &str =
    r"Sensor at x=(?<sx>[0-9]+), y=(?<sy>[0-9]+): closest beacon is at x=(?<bx>\d+), y=(?<by>\d+)";
...
impl FromStr for Sensor {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let sensor_x = caps[0].parse()?;
        let sensor_y = caps[1].parse()?;
        let beacon_x = caps[2].parse()?;
        let beacon_y = caps[3].parse()?;
